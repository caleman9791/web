...
window.onload = function () {
	...
	// ver(nombre, elemento);
	ver("window.indexedDB", window.indexedDB);
	ver("window.IDBTransaction", window.IDBTransaction);
	ver("window.IDBKeyRange", window.IDBKeyRange);

	var DBOpenRequest = window.indexedDB.open('contactsList', 1);
	ver("DBOpenRequest", DBOpenRequest);

	DBOpenRequest.onsuccess = function (event) {
		db = DBOpenRequest.result;
		ver("DBOpenRequest.result", DBOpenRequest.result);
		populateData();
	};
	...
	DBOpenRequest.onupgradeneeded = function (event) {
		...
		var objectStore = db.createObjectStore('contactsList', {
			keyPath: 'id'
		});
		...
		objectStore.createIndex('jTitle', 'jTitle', {
			unique: false
		});
